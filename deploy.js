		...
		req.on('end', () => {
			...
			if (url.pathname === '/deploy') {
				...
				const commands = [
					...
					'cp -r frontend/dist/* /var/www/crypto/ 2>/dev/null || echo "⚠️ Нет файлов для копирования (возможно, сборка не выполнена)"',
					...
					// Перезапуск PM2
					'cd ..',
					'pm2 restart crypto || echo "⚠️ Ошибка перезапуска crypto"',
					`pm2 restart crypto-deploy:${PORT} --update-env || echo "⚠️ Ошибка перезапуска деплой-сервера"`,
				];
